...
class Upload
{
     ...
     * @param string $path The directory in which the file should be moved
     *
     * @return Upload
     */
    public function __construct($file, $path)
    {
        $this->path = $path.'/'.date('d.m.Y');

        $destinationPath = public_path().'/'.$path.'/'.date('d.m.Y');

        $this->filename =
            str_replace(' ', '_', str_random(5).time().$this
                    ->cleanAccents($file->getClientOriginalName()));
    ...
    {
        $img = Image::make(public_path().'/'.$this->path.'/'.$this->filename);
        $img->resize($width, $height);
        $this->filename = 'thumb_'.$this->filename;
        $img->save(public_path().'/'.$this->path.'/'.$this->filename.'/thumbnail');
